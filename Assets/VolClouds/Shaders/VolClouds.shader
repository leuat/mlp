...
#include "AutoLight.cginc"
	...
	float4 integrate( in float4 sum, in float dif, in float den) {
    	float3 l = float3(0.65,0.7,0.75)*_CloudBrightness + float3(1.0, 0.6, 0.3)*dif; 
    	float4 c = float4( lerp( float3(1.0,0.94,0.81), float3(0.251,0.31,0.352), den ), den );
    	c.xyz *= l;
    	c.a *= _CloudHardness;
    	c.rgb *= c.a;
    	return sum + c*(1.0-sum.a);
	}
		...
		for (int i=0;i<N;i++)
			{
				...
				if (intensity > 0.01) {
					...
					sum = integrate(sum, dif, intensity);
				}
				pos = pos + dir*sl;
		}
