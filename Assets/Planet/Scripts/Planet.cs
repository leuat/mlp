...
//		return;
	...
	public void UpdateText() {
		...
		infoTextGO.transform.rotation = GameObject.Find ("CameraLOD").transform.rotation;
...
*/
		...
		d.Scale (Mathf.Min ((float)dist, (float)RenderSettings.LOD_Distance));
		...
		double ds = dist / RenderSettings.LOD_Distance;
            if (ds < 1)
            {
                //                Debug.Log(pSettings.transform.parent.tag);
                //              pSettings.gameObject.transform.parent.tag
            ...
            {
                //            pSettings.gameObject.transform.parent.tag = "LOD";
                tagAll(pSettings.parent, "LOD", 9);
                pSettings.currentTag = "LOD";
                pSettings.currentLayer = 9;

                pSettings.gameObject.transform.localScale = Vector3.one * (float)(1.0 / ds);
            }
