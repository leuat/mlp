...
//			return children!=null;
		...
		public void Realise() {
            ...
            //            RenderSettings.UseThreading = false;
				if (quadGO == null && thread == null) //  && children == null) 
				 	{
...
/*			if (children != null && nestFurther)
			for (int i=0;i<children.Length;i++) {
				min = Mathf.Min(min,children[i].findMinDistance(cam, nestFurther));
			}
			else
			{
/*			for (int i = 0; i < 4; i++) {
				mtmp = (cam - qb.PReal[i].P)/planetSettings.getPlanetSize();
...
//}
		...
		public void Subdivide( int level,  float size) {
			float ang = Vector3.Angle(planetSettings.localCamera.normalized, qb.center.P.normalized);
			if (Mathf.Abs (ang)>90) {
				return;
			}
			
			// HAVE to wait for package to be recieved
			int modifier = 0;
			if (!isEnvironment) {
		//	(int) (Settings.settings.globalVariables.currentSpeed * 0.8);
			}
